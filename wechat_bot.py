        ...
        try:
            ...
            async def handle_send(request):
                try:
                    ...
                    from_id = last_contact["from_id"]
                    context_token = last_contact["context_token"]
                    
                    if not from_id or not context_token:
                        return web.json_response({"success": False, "error": "No active WeChat contact session. Please send a message to the bot first."}, status=400)
                    
                    # 发送消息到当前最近活跃的用户
                    await send_msg_safe(session, from_id, context_token, text, bot_token_ref, bot_base_url_ref)
            ...
            await site.start()
            print("HTTP 本地推送服务器已在 http://127.0.0.1:5001 启动")
        except Exception as e:
            print(f"HTTP 本地推送服务器启动失败: {e}")
